...
- Qeustion 7: Please use your own words to describe the sequence of the library calls that are essential for packet spoofing. This is meant to be a summary
  ...
  - Building and injecting datagrams: sendto(int sockfd, const void *buf, size_t len, int flags, const struct sockaddr &ast; dest_addr, socklen_t addrlen). This library call is used for sending out the IP packet. And len must be the actual size of the packet.
