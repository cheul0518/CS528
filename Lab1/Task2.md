...
- I have two virtual machines run at a time. Let one VM sudo tcpdump ether host 01:02:03:04:05:06 in order to catch a frame from the MAC address. I have another VM sudo ./spoof in order to send an Ethernet frame, and the destination MAC address is the other one's MAC address.
- Below is the result of spoofing an Ethernet Frame, followed by the code.
<img src = "images/part2/eth_frame.png">
...
- Question 4: Can you set the IP packet length field to an arbitary value, regardless of how big the actual packet is?
  - No I cannot. The IP packet length field should be the actual size of the IP packet. If it's not, then the IP packet will not be formed properly. When the packet is sent, it will be truncated because it is too big and is dropped.

- Question 5: Using the raw socket programming, do you have to calculate the checksum for the IP header?
  - The OS will do the job before transmitting the packet over the network. So you don't have to caculate the checksum for the IP header.
...
- Qeustion 7: Please use your own words to describe the sequence of the library calls that are essential for packet spoofing. This is meant to be a summary
  ...
  - Building and injecting datagrams: sendto(int sockfd, const void *buf, size_t len, int flags, const struct sockaddr &ast; dest_addr, socklen_t addrlen). This library call is used for sending out the IP packet. And size_t len must be the actual size of the packet.
